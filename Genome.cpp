...
void Genome::preferentialAttachmentNetwork(const size_t n, size_t e, const double exponent, std::vector<Edge> &edges)
...
void Genome::generateGeneticArchitecture(const ParameterSet& parameters)
{
    ...
    for(size_t crctr = 0u; crctr < parameters.nCharacter; ++crctr) {
        ...
        for(size_t i : vertices[crctr]) {
            characterLocus[i].effectSize = std::gamma_distribution<double>(parameters.alphaAdditive, 1.0)(rnd::rng);
...
void Genome::storeGeneticArchitecture(const std::string &filename, const ParameterSet& parameters)
{
    //open genetic architecture file
