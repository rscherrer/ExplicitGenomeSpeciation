...
#' @param xvariable Optional extra population-wide variable to read from the data files.
#'
...
collect_simulations <- function(
  ...
  # Add optional second variable
  if (!is.null(xvariable)) {
    if (verbose) message("Reading extra data...")
    xdata <- data.frame(do.call("rbind", thislapply(simulations, read_population, xvariable)))
    colnames(xdata) <- timecolumns
    xdata <- xdata %>% gather_("time", xvariable, timecolumns) %>% select(xvariable)
