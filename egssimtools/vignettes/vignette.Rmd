...
The function `plot_trait_values` can be used to produce such density map too, if provided with the dataset with traits as different columns.
...
For ready-made genome scan plots the function `plot_genome_scan` can be used. This function expects a dataset on a per-locus basis, and has multiple options such as displaying the timepoint, showing bars or lollipops instead of points, or even showing an animation through time. Also note that the function `plot_genome_scatter` is an extension of `plot_genome_scan` where the x-axis does not necessarily correspond to locus positions, but can be any locus-specific variable, thus resulting in a 2D scatter plot.

We could also plot Fst through time using a heatmap across the genome:
...
Alternatively we could use the function `plot_genome_heatmap` to do the same.
...
Trajectories of multipe loci through time can also be plotted using the `plot_genome_trajectories` function.
...
Note that the function `plot_genome_network` produces a similar output, with multiple options, including animations!
