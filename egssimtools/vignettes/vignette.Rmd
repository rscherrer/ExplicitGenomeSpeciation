...
Here we provide an overview of the functions of the package. The central function is `read_data`

# Use cases

Here we show how to use the package through a series of examples. You can find all examples in the `scripts/examples.R` script. 
...
```{r}
data <- combine_data(roots, variables = "Fst", by = 3, level = 0, type = "sim")
data
```

The `combine_data` function assembles data from multiple simulations into a single data frame. It internally calls `fetch_dirs`, hence the `level` argument, and so it can go find simulation folders in a nested folder structure provided a certain `pattern` in the naming.

`combine_data` internally calls `read_this`, which is an alias for one of the `read_*` functions. The `type` argument tells `read_this` what type of `read_*` function to become. Here, we make internal use of `read_sim`, for example, which means that `combine_data` will expect you to provide the arguments that `read_sim` expects. See the "Guided tour" section to know more about the `read_*` functions.
...
There is a `check_extant` option that can be used to find out which simulations did succesfully complete and retain only those. It specifically looks for simulations that went extinct before the end, or that did not run at all.
...
Here we want to plot the genome-wide scan of Fst values at the end of the simulation, for each simulation. Again we use `collect_data` to combine data from multiple simulations:
