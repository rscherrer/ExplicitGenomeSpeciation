...
bool Individual::acceptMate(Individual const * const male) const
{
    ...
    // terminate if there is no variation in the sample
    if(var < tiny) return (n > 2u);
    
    scale /= var;
    if(scale > 0.0) {
        // preference towards lower ecotype distance
        size_t k = 0u;
        double delta, sum = 0.0, dxy0 = obs.front(), dxy1;
        // determine threshold for mate acceptance
        for (std::list<double>::iterator it = obs.begin();; ++k) {
            ++it;
            if(it != obs.end()) {
                dxy1 = *it;
                delta = scale * (k + 0.5) * (dxy1 - dxy0);
                if(sum + delta < n) {
                    sum += delta;
                    dxy0 = dxy1;
                }
        ...
        // preference towards higher ecotype distance
        size_t k = 0u;
        double delta, sum = 0.0, dxy0 = obs.back(), dxy1;
        // determine threshold for mate acceptance
        for (std::list<double>::reverse_iterator rit = obs.rbegin();; ++k) {
            ++rit;
            if(rit != obs.rend()) {
                dxy1 = *rit;
                delta = scale * (k + 0.5) * (dxy1 - dxy0);
