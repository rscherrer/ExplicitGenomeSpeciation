...
int doMain(const std::vector<std::string> &args)
{
    ...
    try
    {
        ...
        // GeneticArchitecture arch = GeneticArchitecture(
        // pars.getNChromosomes(), pars.getNTraits(),
        //        pars.getNLociPerTrait(), pars.getNEdgesPerTrait(),
        //pars.getSkewnesses());
        ...
        if (parameters.isGenerateArchitecture)
        {
            ...
            std::clog << "New genetic architecture saved as " <<
parameters.architectureFileName
        ...
        else
        {
            ...
            // Otherwise load the genetic architecture from a genetic
architecture file
        ...
        for (int t = 1 - parameters.tBurnIn; t <= parameters.tEndSim; ++t) {
            ...
            if (isBurnin) {
                ...
                // Open question: is is better to keep all individuals in a
single vector
                // and order them by habitat,
                // Or have two separate vectors, subpopulations 1 and 2,
members of class
                // Population?
